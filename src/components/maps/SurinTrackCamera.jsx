...
const SurinTrackCamera = () => {
  const mapContainer = useRef(null)

  useEffect(() => {
    const loadFiles = [
      fetch('/maps/surin-seville.geojson').then((response) => response.json()),
      fetch('/maps/camera-seville.geojson').then((response) => response.json()),
    ]
    ...
    Promise.all(loadFiles).then(function (data) {
      ...
      map.on('load', function () {
        ...
        function frame() {
          ...
          const rate = y / window.innerHeight < 0.03 ? 0.03 : y / window.innerHeight
          const cameraAltitude =
            rate <= 0.03 ? cameraAltitude_base + (0.0305 - rate) * 100000 : cameraAltitude_base + rate * rate * 10000

          const alongRoute = turf.along(turf.lineString(shortTargetRoute), routeLength * rate).geometry.coordinates

          const alongCamera = turf.along(turf.lineString(cameraRoute), cameraRouteLength * rate).geometry.coordinates
...
export default SurinTrackCamera
